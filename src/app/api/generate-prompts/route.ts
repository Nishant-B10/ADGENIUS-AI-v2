...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // *** FIX 1: The 'content' string was not a template literal. ***
    // It was using single quotes ('...'), which prevents variables like ${productName}
    // from being injected. Changed to backticks (`) to enable template interpolation.
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-3-sonnet-20240229', // Using a standard, widely available model name
        max_tokens: 4000,
        temperature: 0.7,
...
Return ONLY valid JSON using "${productName}" throughout. Do not wrap the JSON in markdown code blocks.
`
        }]
      })
    });
    ...
    try {
      // Clean the response in case the model still adds markdown wrappers
      ...
      console.error('Raw Claude Response:', responseText); // Log raw response for debugging
    ...
    // *** FIX 2: Re-declared all necessary variables for the fallback block. ***
    // The original `catch` block couldn't access variables from the `try` block.
    ...
    // *** FIX 3: Corrected JSON syntax and used template literals for all strings. ***
    // The original fallback object was not valid JSON and used static strings.
    return NextResponse.json({
      success: false,
      ...
      data: {
        storyboard: {
          ...
          scenes: [
            ...
            },
            {
              ...
              action: `Final hero shot of ${productName} with a call to action`,
              lighting: `Studio lighting with ${primaryColor} and ${accentColor} accents`,
              subject: `The ${productName} product, clean and pristine`,
              brand_integration: `${productName} logo and tagline displayed clearly`,
        ...
        ad_storyline: {
          ...
          demographic_targeting: `${productName} story that resonates with the ${targetAges} ${incomeGroups} audience`,
          visual_progression: `Visuals progress with brand colors: ${secondaryColor} → ${primaryColor} → ${accentColor}`,
          product_integration: `${productName} is featured prominently throughout the narrative arc`
        }
      }
    }, { status: 500 });
  }
